...
export function ComoFunciona() {
  return (
    <section id="como-funciona" className="bg-[var(--color-bone)] border-b-2 border-[var(--color-ink)]">
      <div className="mx-auto max-w-7xl px-5 md:px-8 py-16 md:py-24">
        ...
        <div className="relative grid md:grid-cols-4 gap-px bg-[var(--color-ink)] border-2 border-[var(--color-ink)]">
          {STEPS.map((s) => (
            <div key={s.n} className="bg-[var(--color-bone)] p-6 md:p-8 flex flex-col">
              ...
              <h3 className="font-display text-lg md:text-xl font-bold text-[var(--color-ink)] uppercase tracking-wide">
                {s.t}
              </h3>
              <p className="mt-2 text-sm text-[var(--color-ink)]/70 leading-relaxed">{s.d}</p>
            </div>
          ))}
        </div>
      </div>
